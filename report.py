...
def add_report_to_db(rep):
    conn = sqlite3.connect(REPORT_DB)
    cur = conn.cursor()
    cur.execute('\n    INSERT INTO reports (timestamp, rank, reporter, reporter_profile, reported, reported_profile, reason, result, jobid, raw)\n    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)\n    ', (rep.get('timestamp'), rep.get('rank'), rep.get('reporter'), rep.get('reporter_profile'), rep.get('reported'), rep.get('reported_profile'), rep.get('reason'), rep.get('result'), rep.get('jobid'), rep.get('raw')))
    conn.commit()
    rowid = cur.lastrowid
    conn.close()
    return rowid
...
def add_ban_to_db(ban):
    conn = sqlite3.connect(BAN_DB)
    cur = conn.cursor()
    cur.execute('\n    INSERT INTO bans (timestamp, rank, user, reason, punishment, proof, report_id)\n    VALUES (?, ?, ?, ?, ?, ?, ?)\n    ', (ban.get('timestamp'), ban.get('rank'), ban.get('user'), ban.get('reason'), ban.get('punishment'), ban.get('proof'), ban.get('report_id')))
    conn.commit()
    rowid = cur.lastrowid
    conn.close()
    return rowid
...
def count_reports_for_player(player_name_or_id):
    ...
    conn.close()
    return cnt
PROFILE_URL_RE = re.compile('https?://www\\.roblox\\.com/users/(\\d+)/profile', re.I)
JOBID_RE = re.compile('JobID[:\\s]*([0-9a-fA-F\\-]{8,})', re.I)
...
def parse_raw_report(text):
    ...
    for i, l in enumerate(lines):
        if l.lower().startswith('reason'):
            for j in range(i + 1, len(lines)):
                stop_terms = ('jobid', 'result', 'reporter', 'player being reported', 'reported', 'new report', 'report type', 'player')
                lj = lines[j].lower()
    ...
    if not reason:
        ...
        reason = '\n'.join(reason_lines).strip()
    return {'reporter': reporter, 'reporter_profile': reporter_profile, 'reported': reported, 'reported_profile': reported_profile, 'reason': reason, 'jobid': jobid}
...
class ReportApp:
    ...
    def on_punishment_click(self, name):
        ...
        m = re.search('^Reported:\\s*(.+)$', curr_rep, re.MULTILINE)
        if m:
    ...
    def generate_report(self):
        ...
        rep = {'timestamp': timestamp, 'rank': self.rank_var.get(), 'reporter': reporter, 'reporter_profile': parsed.get('reporter_profile'), 'reported': reported, 'reported_profile': parsed.get('reported_profile'), 'reason': reason, 'result': result, 'jobid': jobid, 'raw': raw}
        report_id = add_report_to_db(rep)
        ban = {'timestamp': timestamp, 'rank': self.rank_var.get(), 'user': reported, 'reason': reason, 'punishment': punishment or '', 'proof': '', 'report_id': report_id}
        add_ban_to_db(ban)
    ...
    def _auto_send_webhook(self, url, report_text, ban_text, timestamp_iso):
        if not requests:
            ...
            return
        payload = {'username': self.rank_var.get(), 'embeds': [{'title': 'Report Log', 'description': f'```{report_text}```', 'timestamp': timestamp_iso}, {'title': 'Punishment Log', 'description': f'```{ban_text}```', 'timestamp': timestamp_iso}]}
        try:
